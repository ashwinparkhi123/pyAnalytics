...
sleep.isna().sum(axis=1) #1=row
sleep.columns
sleep.isna().sum(axis=0) #0=column
...
sleep.dropna(thresh=9, axis=0, inplace=False).shape #0=row list out those data which have got atleast data in 9 columns
sleep.dropna(thresh=55, axis=1, inplace=False).shape #1-column retain which column have 55 non-na values
